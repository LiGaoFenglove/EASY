...
//react-router4.0 规定路由信息只能有一层。比如容器组件可以直接调用路由里面的信息，
//而UI组件（容器组件调用UI组件）隔了一层容器组件（相当于和路由隔了两层关系，
//所以不能直接调用）  所以不能直接调用路由里面的信息。所以要引用 withRouter 这个方法
//组件必须放在<BrowserRouter>里面这个在app/APP.js文件下
 ...
 export default class Header extends Component {
	...
	render() {
		...
		return (
			...
			<Row>
		      <Col span={4}>
		      	<img src={logo} onClick={this.handleClick.bind(this)} className={styles.logo} alt=" "/>
		      </Col>
		      <Col span={20}>
		      	{/*mode="horizontal"  横向排列的意思*/}
		      	<Menu mode="horizontal">
...
//import 'whatwg-fetch'  在app 里面引用一下  全局可用
	...
	handleClick() {
		console.log(this.props)//这里有history下的replace方法
